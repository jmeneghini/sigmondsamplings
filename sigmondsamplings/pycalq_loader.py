...
class PyCALQLoader(SigmondLoader):
    ...
    def __init__(self, pycalq_project_base_dir: Optional[str | Path] = None, hdf5_path: str = "/samplings"):
        """
        ...
        """
        pycalq_project_base_dir = Path(pycalq_project_base_dir) if pycalq_project_base_dir else Path.cwd()
    ...
    def get_estimate_files(self,
                           result_type: Optional[PyCALQEstimateResultType |
                                        List[PyCALQEstimateResultType]] = None) -> List[str]:
        """
        ...
        for rtype in result_type:
            files = self.pycalq_project_base_dir.glob(f"3fit_spectrum/data/estimates/*-{rtype.value}_estimates.csv")
            estimate_files.extend([str(f) for f in files])  
        
        return estimate_files

    def get_estimate_file_path(self,
                               ...
                               resampling_method: Optional[str] = None) -> str | List[str]:
        """
        ...
        def build_glob_pattern(rtype: PyCALQEstimateResultType) -> str:
            ...
            return str(Path(self.pycalq_project_base_dir) / "3fit_spectrum" / "data" / "estimates" / filename)
    ...
    def get_sampling_files(self,
                           result_type: Optional[PyCALQSamplingResultType |
                                        List[PyCALQSamplingResultType]] = None) -> List[str]:
        """
        ...
        if result_type is None:
            result_type = [PyCALQSamplingResultType.ENERGY_LEVELS,
        ...
        sampling_files = []
        for rtype in result_type:
            files = self.pycalq_project_base_dir.glob(f"3fit_spectrum/data/samples/fit_spectrum_{rtype.value}*.hdf5")
            sampling_files.extend([str(f) for f in files])

        return sampling_files

    def get_sampling_file_path(self,
        ...
        def build_glob_pattern(rtype: PyCALQSamplingResultType) -> str:
            ...
            return str(Path(self.pycalq_project_base_dir) / "3fit_spectrum" / "data" / "samples" / filename)
    ...
    def get_all_available_tags(self) -> List[str]:
        ...
        if not full_sampling_dir_path.exists():
            raise FileNotFoundError(f"PyCALQ sampling directory not found: {full_sampling_dir_path}")
    ...
    def get_file_data_type(self,
                           file_path: str) -> PyCALQSamplingResultType | PyCALQEstimateResultType:
        """
    ...
    def get_file_name_tag(self, file_path: str) -> str:
        ...
        # and ends at '-Nbin'.
            ...
            tag_match = re.search(rf"fit_spectrum_{self.get_file_data_type(file_name).value}-(.*?)-Nbin", file_name)
    ...
    def load_observables_by_tag(self, 
                                tag: str,
                                result_types: Optional[List[PyCALQSamplingResultType]] = None,
                                resampling_method: Optional[str] = None,
                                pivot_type: Optional[PyCALQPivotType] = None,
                                rotate_info: Optional[PyCALQRotateInfo] = None,
                                num_bins: Optional[int] = None) -> Dict[str, Dict[str, 'SigmondSampling']]:
        """
    ...
    def load_interacting_estimates_to_dataframe(self,
                                      tag: str,
                                      resampling_method: Optional[str] = None,
                                      pivot_type: Optional[PyCALQPivotType] = None,
                                      rotate_info: Optional[PyCALQRotateInfo] = None,
                                      num_bins: Optional[int] = None) -> Any:
        """
    ...
    def load_NIs_to_dict(self,
                        tag: str,
                        resampling_method: Optional[str] = None,
                        pivot_type: Optional[PyCALQPivotType] = None,
                        rotate_info: Optional[PyCALQRotateInfo] = None,
                        num_bins: Optional[int] = None) -> Dict[int, List[List[str]]]:
        """
    ...
    def load_single_hadron_estimates_to_dataframe(self,
                                      tag: str,
                                      resampling_method: Optional[str] = None,
                                      pivot_type: Optional[PyCALQPivotType] = None,
                                      rotate_info: Optional[PyCALQRotateInfo] = None,
    ...
    def load_energy_levels(self, 
                          ...
                          pivot_type: Optional[PyCALQPivotType] = None,
                          rotate_info: Optional[PyCALQRotateInfo] = None,
                          num_bins: Optional[int] = None) -> Dict[str, 'SigmondSampling']:
        """
    ...
    def load_energy_levels_and_build_mom_dict(self,
                                              tag: str,
                                              resampling_method: Optional[str] = None,
                                              pivot_type: Optional[PyCALQPivotType] = None,
                                              rotate_info: Optional[PyCALQRotateInfo] = None,
                                              ...
                                              ref: Optional[bool] = True) -> Dict[int, Dict[int, Tuple[List[str],
                                                                        Dict[str, SigmondSampling]] | Dict[str, SigmondSampling]]]:
        """
    ...
    def load_operator_overlaps(self, 
                              tag: str,
                              resampling_method: Optional[str] = None,
                              pivot_type: Optional[PyCALQPivotType] = None,
                              rotate_info: Optional[PyCALQRotateInfo] = None,
    ...
    def load_fit_parameters(self, 
        ...
        Dict[str, SigmondSampling]
            Dictionary mapping observable names to SigmondSampling objects.
        """
        if parameter_type not in [PyCALQSamplingResultType.INTERACTING_FIT_PARAMETERS, 
                                 PyCALQSamplingResultType.SINGLE_HADRON_FIT_PARAMETERS]:
            raise ValueError("parameter_type must be INTERACTING_FIT_PARAMETERS or SINGLE_HADRON_FIT_PARAMETERS")
    ...
    def get_available_files_info(self, tag: Optional[str] = None) -> List[Dict[str, Any]]:
        """
    ...
    def find_files_by_criteria(self, 
                              tag: Optional[str] = None,
                              result_type: Optional[PyCALQSamplingResultType | PyCALQEstimateResultType] = None,
                              resampling_method: Optional[str] = None,
                              pivot_type: Optional[PyCALQPivotType] = None,
                              rotate_info: Optional[PyCALQRotateInfo] = None,
                              num_bins: Optional[int] = None) -> List[str]:
        """
        ...
        for file_info in all_files:
            ...
            if result_type is not None and file_info['data_type'] != result_type:
                match = False
            if resampling_method is not None and file_info['resampling_method'] != resampling_method:
                match = False
            ...
            if rotate_info is not None:
                ...
                if (file_rotate is None or 
                    file_rotate.tN != rotate_info.tN or 
                    file_rotate.t0 != rotate_info.t0 or 
                    file_rotate.tD != rotate_info.tD):
                    match = False
    ...
    @staticmethod
    def parse_observable_name(obs_name: str, ref: bool = False) -> Optional[Tuple[int, str, int]]:
        """
