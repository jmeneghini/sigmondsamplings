...
def sample_observables(sample_ensemble_info):
    ...
    return [
        ObservableInfo(
            "isosinglet_S=0_A1g_1_PSQ=0_elab_2_ref",
            0,
            "mcobs",
            "re",
            sample_ensemble_info,
        ),
        ObservableInfo(
            "isosinglet_S=0_A1_1_P=(0,0,1)_elab_2_ref",
            0,
            "mcobs",
            "re",
            sample_ensemble_info,
        ...
        ObservableInfo(
            "isosinglet_S=0_A1_1_P=(0,1,1)_elab_3_ref",
            0,
            "mcobs",
            "re",
            sample_ensemble_info,
...
def sample_k_elements():
    ...
    return [KElementInfo(0, "L(0) 2S(0) chan(0)", "L(0) 2S(0) chan(0)")]


@pytest.fixture
def sample_decay_channels():
    """Create sample decay channels for testing."""
    return [ParticleInfo("phi", 0, True)]
...
def sample_fit_forms():
    ...
    return {
        "L(0) 2S(0) chan(0)": ExpressionFitForm(
            "0.5*sqrt(x^2 - 4.0)*(rho_mass_ref^2 - x^2)/(rho_mass_ref * Gamma_ref)"
        )
    }
...
class TestKBfitXMLHelper:
    ...
    def test_create_root_finder_config(self, kb_helper):
        ...
        config = RootFinderConfig(abs_x_tolerance=1e-7, min_step_percent=1e-6)
    ...
    def test_create_kbblock_detres(
        self, kb_helper, sample_ensemble_info, sample_box_quantizations
    ):
    ...
    def test_create_kbblock_spectrum(
        self, kb_helper, sample_ensemble_info, sample_box_quantizations
    ):
    ...
    def test_create_kbobservables(
        self, kb_helper, sample_sampling_info, sample_ensemble_info
    ):
    ...
    def test_create_ktilde_matrix_inverse(
        self,
        kb_helper,
        sample_k_elements,
        sample_fit_forms,
        sample_decay_channels,
    ...
    def test_create_detres_xml(
        self,
        kb_helper,
        sample_observables,
        sample_sampling_info,
        sample_box_quantizations,
        sample_k_elements,
        sample_fit_forms,
        sample_decay_channels,
        sample_fit_parameters,
    ...
    def test_create_spectrum_xml(
        ...
        cm_energy_ranges = {0: (2.0, 3.0), 1: (2.1, 3.1)}
    ...
    def test_create_print_xml(
        ...
        energy_ranges = {0: (2.0, 4.0, 0.01), 1: (2.5, 4.5, 0.01)}
    ...
    def test_fit_form_validation(self):
        ...
        with pytest.raises(
            ValueError, match="Either degree or powers must be specified"
        ):
            PolynomialFitForm()
        ...
        with pytest.raises(
            ValueError, match="Either number_of_poles or pole_indices must be specified"
        ):
            SumOfPolesFitForm()

        with pytest.raises(
            ValueError, match="Cannot specify both number_of_poles and pole_indices"
        ):
