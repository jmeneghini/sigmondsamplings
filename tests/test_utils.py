...
from SigmondSamplings.utils import (
    create_gaussian_sampling, create_uniform_sampling, create_complex_gaussian_sampling,
    bootstrap_resample, jackknife_resample, combine_real_imaginary, 
    split_complex_sampling, effective_sample_size, block_average
)
from SigmondSamplings.sampling import SigmondSampling, ObservableInfo, EnsembleInfo, SamplingInfo
...
class TestSyntheticSamplings(unittest.TestCase):
    ...
    def test_create_gaussian_sampling(self):
        ...
        sampling = create_gaussian_sampling(mean=1.0, std=0.1, sampling_info=sampling_info)
    ...
    def test_create_uniform_sampling(self):
        ...
        sampling = create_uniform_sampling(low=0.5, high=1.5, num_samples=100, seed=1234)
...
class TestComplexOperations(unittest.TestCase):
    ...
    def test_combine_real_imaginary(self):
        ...
        complex_sampling = combine_real_imaginary(self.real_sampling, self.imag_sampling)
    ...
    def test_combine_incompatible_error(self):
        ...
        different_obs_info = ObservableInfo("different_obs", 0, 'n', 'im', different_ensemble)
    ...
    def test_split_complex_sampling(self):
        ...
        complex_obs_info = ObservableInfo("complex_obs", 0, 'n', 'cx', self.ensemble_info)
        complex_sampling = SigmondSampling(
            complex_data, complex_obs_info, self.sampling_info, 
            is_complex=True
        )
...
class TestStatisticalFunctions(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for i in range(1, 1000):
            self.correlated_data[i] = 0.5 * self.correlated_data[i-1] + np.random.normal(0, 1)
...
class TestUtils(unittest.TestCase):
    ...
    def test_create_complex_gaussian_sampling(self):
        ...
        sampling_info = SamplingInfo("bootstrap", 100, 1234)
        sampling = create_complex_gaussian_sampling(1.0, 0.1, 2.0, 0.2, 
                                                   sampling_info)
    ...
    def test_split_complex_basic(self):
        ...
        complex_sampling = SigmondSampling(complex_data, complex_obs_info, sampling_info, 
                                          is_complex=True)
...
class TestCreateAndCombineComplexSampling(unittest.TestCase):
    ...
    def test_create_and_combine(self):
        ...
        sampling_info = SamplingInfo("bootstrap", 10, 1234)
        complex_sampling = create_complex_gaussian_sampling(1.0, 0.1, 2.0, 0.1, 
                                                          sampling_info)
        self.assertTrue(complex_sampling.is_complex)
