...
class TestSigmondLoader(unittest.TestCase):
    ...
    def test_check_file_validity_fstream(self):
        ...
        loader = SigmondLoader()
        is_valid, file_type, hdf5_paths = loader.check_file_validity(
            self.test_files["fstream"]
        )
    ...
    def test_check_file_validity_hdf5_no_path(self):
        ...
        loader = SigmondLoader()
        is_valid, file_type, hdf5_paths = loader.check_file_validity(
            self.test_files["hdf5_without_path"]
        )
    ...
    def test_check_file_validity_hdf5_with_path(self):
        ...
        loader = SigmondLoader()
        is_valid, file_type, hdf5_paths = loader.check_file_validity(
            self.test_files["hdf5_with_path"]
        )
    ...
    def test_get_file_info_valid_hdf5(self):
        ...
        loader = SigmondLoader()
        ensemble_info, sampling_info, observable_infos = loader.get_file_info(
            self.test_files["valid_hdf5_with_path"]
        )
    ...
    def test_check_file_validity_valid_hdf5_no_path(self):
        ...
        loader = SigmondLoader()
        is_valid, file_type, hdf5_paths = loader.check_file_validity(
            self.test_files["valid_hdf5_without_path"]
        )
    ...
    def test_get_file_info_jackknife_hdf5(self):
        ...
        loader = SigmondLoader()
        ensemble_info, sampling_info, observable_infos = loader.get_file_info(
            self.test_files["jackknife_hdf5_with_path"]
        )
        ...
        self.assertEqual(
            sampling_info.num_resamplings, 28000
        )  # For jackknife, num_resamplings = num_bins
    ...
    def test_check_file_validity_jackknife_hdf5_no_path(self):
        ...
        loader = SigmondLoader()
        is_valid, file_type, hdf5_paths = loader.check_file_validity(
            self.test_files["jackknife_hdf5_without_path"]
        )
    ...
    def test_find_observables(self):
        ...
        matches = loader.find_observables(
            self.test_files["fstream"], name_pattern="kaon"
        )
    ...
    @unittest.skip(
        "Skip loading full data for faster tests - enable for full integration testing"
    )
    def test_load_observable(self):
        ...
        if observable_infos:
            ...
            sampling = loader.load_observable(
                self.test_files["fstream"], observable_infos[0]
            )
            ...
            self.assertEqual(
                len(sampling.resampled_values), sampling_info.num_resamplings
            )

    @unittest.skip(
        "Skip loading all data for faster tests - enable for full integration testing"
    )
